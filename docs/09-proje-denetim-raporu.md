  ...
  ...tseslint.configs.stylisticTypeChecked,
  {
    ...
    rules: {
      ...
      'no-restricted-imports': ['error', {
        patterns: [{ group: ['@pusula/llm-gateway/src/adapters/*'], message: 'adapter\'lar yalnız gateway üzerinden' }],
      }],
    },
  },
);
...
export class ComparablesRepository {
  ...
  async forKonut(input: KonutInput): Promise<KomparableIlan[]> {
    ...
    const m2Lo = Math.floor(m2 * 0.8), m2Hi = Math.ceil(m2 * 1.2);
    const yasLo = input.bina_yasi - 5, yasHi = input.bina_yasi + 5;
    ...
    const { data, error } = await this.sb
      ...
      .gte('net_m2', m2Lo).lte('net_m2', m2Hi)
      .gte('bina_yasi', Math.max(0, yasLo)).lte('bina_yasi', yasHi)
      .gte('parse_tarihi', new Date(Date.now() - 90 * 86400_000).toISOString())
      .neq('id', input.kaynak_id) // kendini dahil etme
      .limit(50);
    if (error) throw error;
    return (data ?? []).map((r) => ({
      id: r.id, m2: r.net_m2, fiyat_tl: r.fiyat_tl,
      bina_yasi: r.bina_yasi, oda_sayisi: r.oda_sayisi,
      mahalle: r.mahalle ?? undefined, ilce: r.ilce,
    }));
  }
}
```

...
# Build command guvenli versiyon
$buildCmd = "corepack enable && pnpm install --no-frozen-lockfile && pnpm --filter @pusula/shared build && pnpm --filter @pusula/scoring build && pnpm --filter @pusula/llm-gateway build && pnpm --filter @pusula/api build"
...
Write-Host "Build command güncelleniyor..."
try {
...
# Manuel redeploy
Write-Host "Redeploy tetikleniyor..."
try {
